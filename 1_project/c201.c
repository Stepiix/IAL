...
void List_Dispose( List *list ) {
		while(list->firstElement != NULL){ //while list isnt empty
		struct ListElement *pointer = NULL;
		pointer = list->firstElement;
		list->firstElement = list->firstElement->nextElement; //first element is next element
		free(pointer); //delete first element
	}
	list->activeElement = NULL;
	list->firstElement = NULL;
}
